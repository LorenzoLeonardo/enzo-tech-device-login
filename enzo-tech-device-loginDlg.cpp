...
void SendPostJsonRequest(CString action, CString userId) {
    ...
    CString jsonData;
    jsonData.Format(
        _T(R"({"username":"%s","timestamp":"%s","action":"%s","user_id":"%s","device_id":"%s"})"),
    ...
    // Connect to host
    HINTERNET hConnect =
        InternetConnect(hInternet, _T("enzotechcomputersolutions.com"), INTERNET_DEFAULT_HTTPS_PORT,
    ...
    HINTERNET hRequest = HttpOpenRequest(
        hConnect, _T("POST"), _T("/device_login"), NULL, NULL, NULL,
        ...
        try {
            ...
            if (success) {
                ...
                if (errorCode == "user_not_found") {
                    ...
                    errorMessage.Format(_T("%s. Would you like to register?"),
                    ...
                    if (result == IDYES) {
                        ...
                        ShellExecute(NULL, _T("open"),
                                     _T("https://enzotechcomputersolutions.com/login"), NULL, NULL,
                                     SW_SHOWNORMAL);
                    }
                } else {
                    CString errorMessage;
...
void SendGetJsonRequest(CString userId) {
    ...
    HINTERNET hRequest = HttpOpenRequest(
        hConnect, _T("GET"), request, NULL, NULL, NULL,
        INTERNET_FLAG_RELOAD | INTERNET_FLAG_NO_CACHE_WRITE | INTERNET_FLAG_SECURE, 0);
        ...
        try {
            ...
            if (successFlag) {
                CString message = (userId == _T("login"))    ? _T("Login successful")
                                  : (userId == _T("logout")) ? _T("Logout successful")
                                                             : _T("Success");
                AfxMessageBox(message, MB_OK | MB_ICONINFORMATION);
            } else {
                if (errorCode == "user_not_found") {
                    CString prompt;
                    prompt.Format(_T("%s. Would you like to register?"),
                                  (LPCTSTR)CString(CA2T(error.c_str())));
